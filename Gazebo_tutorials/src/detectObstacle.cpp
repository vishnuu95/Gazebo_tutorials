...
detectObstacle::detectObstacle(float threshold, float vel, float omega) {
	...
	this->msg;
	this->myPublisher = this->n.advertise<geometry_msgs::Twist>("/cmd_vel", 1000);
	this->mySubscriber = this->n.subscribe<sensor_msgs::LaserScan>("/scan", 1000, &detectObstacle::scanSpace, this);
	this->rightFov = 65;
	this->leftFov = 65;
}
